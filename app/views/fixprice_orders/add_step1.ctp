...
<div class="fixpriceOrders form fillupform">
		...
		<table width="100%" class="table_bds_form map_left_intro">
			...
			<tr>
				<td>
					<?php
						echo $this->Form->input('Product.name', array('label'=>'Tiêu đề', 'class'=>'required', 'placeholder'=>'Tiêu đề cho yêu cầu thẩm định (Ví dụ: cần mua nhà để ở tại quận...)'));			
					?>
				</td>				
			</tr>
			<tr>
		...
		<div id="ForHouse" class="forhouse">
									...
									<div class="box_b">
									    ...
									    <?php echo str_replace(array(':', '-'), '', $this->Form->input('Product.build_date', array('label'=>'Thời gian xây (ngày/tháng/năm)', 'placeholder'=>'ngày/tháng/năm','monthNames' => false,
							'dateFormat' => 'DMY',
							'minYear' => date('Y') - 50,
							'maxYear' => date('Y') ))); ?>
		...
		<div id="Forland" class="forhouse box_b">
				...
				<?php
				    echo $this->Form->input('Product.area_x', array('label'=>'Diện tích khuôn viên', 'after'=>'(m)', 'placeholder'=>'Ngang'));
				    echo $this->Form->input('Product.area_y', array('label'=>'', 'after'=>'(m)', 'placeholder'=>'Dài'));
				?>
...
<div class="des_img">	
			...
			<div style="clear: both"><strong>(Tải hình ảnh - tập tin ảnh JPG/PNG không quá 2MB)</strong></div>
			<p class="uppic_note">Để đảm bảo hình ảnh đầy đủ và khách quan, bạn phải nhập đủ 3 ảnh của bất động sản bao gồm: hình ảnh bất động sản (nhìn chính diện từ bên ngoài), hình ảnh bên trong bất động sản, đường(hẻm) trước nhà.</p>
		...
		<div class="pdescription">
			<?php echo $this->Form->input('Product.description', array( 'style'=>'width:100%;height:200px', 'label'=>'Mô tả thêm các đặc điểm khác của bất động sản'));	 ?>
		</div>

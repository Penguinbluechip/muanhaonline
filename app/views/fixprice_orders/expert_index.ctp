<div class="fixpriceOrders index">
	...
	<tr<?php echo $class;?>>
		...
		<td class="actions">
			...
			<?php if($fixpriceOrder['FixpriceOrderState']['alias'] == 'EXPERT_CONFIRMED' && $isLeader && !$fixpriceOrder['FixpriceOrder']['assigner_id']){
				echo $this->Html->link(__('Bàn giao', true), array('controller'=>'fixprice_orders','action' => 'assign', $fixpriceOrder['FixpriceOrder']['id']));
			} ?>
			<?php if(isset($fixpriceOrder['remain_hours']) && $fixpriceOrder['remain_hours'] > 0) if(($fixpriceOrder['FixpriceOrderState']['alias'] == 'EXPERT_CONFIRMED' || $fixpriceOrder['FixpriceOrderState']['alias'] == 'INVALID') && $user["id"] == $fixpriceOrder['FixpriceOrder']['expert_id']) { ?>
			...
			<?php } ?>
			<?php if(isset($fixpriceOrder['remain_hours']) && $fixpriceOrder['remain_hours'] > 0) if($fixpriceOrder['FixpriceOrderState']['alias'] == 'EXPERT_CONFIRMED' && $user["id"] == $fixpriceOrder['FixpriceOrder']['assigner_id']) { ?>

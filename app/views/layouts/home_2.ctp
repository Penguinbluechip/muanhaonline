...
function updateCurrency()
{
	...
	$(function() {
		...
		//Log box
		$('#loginbox .loginbox_bg, #loginbox .close_but').click(function() {
			$('#loginbox').fadeOut();
		});
		...
		<div style="" class="login-inner">
			...
			<div class="fillupform">
					...
					<?php if(empty($user)) { ?>
						...
						<p>Bạn phải đăng nhập để có thể sử dụng dịch vụ này.<br />Nếu chưa có tài khoản thành viên, bạn cần <a href="<?php echo $this->Html->url(array('controller'=>'users', 'action'=>'register')) ?>" ><strong>Đăng ký</strong></a> với chúng tôi.</p>
						<?php

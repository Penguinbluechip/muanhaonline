...
<h2 style="clear:both"><?php __('Sửa thông tin sản phẩm');?>: <?php echo $this->data["Product"]["name"] ?></h2>
...
<table>
	...
	<div id="tabs-1">
		...
		<?php
			...
			//echo strtotime($this->data["Product"]["for"]) - strtotime(date('Y-m-d H:i:s'));
			...
			?>
					...
					<select id="ProductPayStatus" name="data[Product][pay_status]">
						<?php
						$directions = array(array("0", __("tin thường", true)),array("1", __("tin VIP", true)));
						foreach($directions as $f)
						{
							$selected = '';
							if($this->data["Product"]["pay_status"] == $f[0])
							{
			...
			<div class="input text required">
				...
				<select id="ProductFor" name="data[Product][for]">
					<?php
					$fors = array(array("s", __('Cần bán', true)),array("l", __('Cho thuê', true)),array("m", __('Cần mua', true)));
					foreach($fors as $f)
					{
						$selected = '';
			...
			?>
				...
				<div class="input text required">
					<div class="facade_input"><input type="radio" id="ProductFacade" name="data[Product][facade]" value="Hẻm" <?php if($this->data["Product"]["facade"] == "Hẻm") echo ' checked="checked"' ?> /> Hẻm</div>
					<div class="facade_input"><input type="radio" id="ProductFacade" name="data[Product][facade]" value="Mặt tiền" <?php if($this->data["Product"]["facade"] == "Mặt tiền") echo ' checked="checked"' ?> /> Mặt tiền</div>						
				</div>
			...
			<td>			
				...
				<div style="text-align: center">(Nhấn nút 'tìm trên bản đồ'. Sau đó kéo & thả mũi tên màu đỏ trên bản đồ để xác định chính xác vị trí BĐS)</div>
			</td>
		</tr></table>
	...
	<div id="tabs-2">	
		<table><tr>
			...
			<div id="ForHouse">
			    ...
			    <!--<div class="input text">
				    <label for="ProductFloor"><?php echo __('Lầu'); ?></label>
				    
				    <select id="ProductFloors" name="data[Product][floors]">
					    <?php
					    $floors = array(array("trệt", "trệt"),array("1", "1"),array("1.5", "1.5"),array("2", "2"),array("2.5", "2.5"),array("3", "3"),array("3.5", "3.5"),array("4", "4"),array("4.5", "4.5"),array("5", "5"),array("gt5", 'trên 5'));
					    foreach($floors as $f)
					    {
						    $selected = '';
			...
			<div id="Forland">
				...
				<?php
				   echo $this->Form->input('area_x', array('label'=>'Ngang', 'after'=>'(m)'));
				    ...
				    echo $this->Form->input('area_back', array('label'=>'Nở hậu', 'after'=>'(m)'));
				?>
			    </div>
			</div>
			...
			//echo $this->Form->input('expire_date', array('label'=>'Ngày hết hạn','monthNames' => false));
		...
		<td>
			...
			<div id="image-list">
					...
					<div class="meta">
				<?php echo $this->Form->input('meta_keywords', array('label'=>'Meta Keywords', 'rows'=>'3')); ?>
				<?php echo $this->Form->input('meta_description', array('label'=>'Meta Description')); ?>
			</div>
        ...
        <div style="display: none">
		  <?php echo $this->element('whoisonline'); ?>
                                ...
                                <div class="submit_buts">
                                    <?php echo $form->submit('Xem trước tin', array('name'=>'preview', 'onclick'=>"window.open('', preview, 'height=700,width=1000,scrollbars=yes');this.form.target=preview;return true;")); ?>
				    <?php echo $form->submit('Lưu tin', array('onclick'=>"this.form.target='_self';return true;")); ?>
                                    <?php echo $form->submit('Lưu & Đăng tin', array('name'=>'postnew', 'onclick'=>"this.form.target='_self';return true;")); ?>

...
<table style="width:auto">
    <tr>
	<td><label for="CityId"><?php __('Tên'); ?></label></th>       

	<td><input type="text" name="filter_keyword1" class="large" value="<?php echo $keyword; ?>" /></td>
    </tr>
    <tr>
	<td><label for="CityId"><?php __('Thể loại'); ?></label></th>       

	<td>
	    <select id="CategoryId" name="filter_category_id1" onchange="">
                    <option value="0">- <?php echo __("loại dự án"); ?> -</option>
                        <?php foreach($projectCategories as $c ) {
                            $selected = "";
                            if($c["ProjectCategory"]["id"] == $category_id)
                            {
                                $selected = 'selected="selected"';
                            }
                        ?>
                            <option value="<?php echo $c["ProjectCategory"]["id"] ?>" <?php echo $selected; ?> ><?php echo $c["ProjectCategory"]["name"] ?></option>
                        <?php } ?>
                </select>
	</td>
    </tr>
...
<table style="width:auto">
    ...
    <tr>
		...
		<li class="buttons clear"><a onclick="$('#filter').submit();" href="#submit"><span>Tìm kiếm</span></a>
		    <a onclick="" href="<?php echo $this->Html->url(array('controller'=>'projects', 'action'=>'reset')) ?>"><span>Xóa bộ lọc</span></a>
		    </li>
	    </ul>

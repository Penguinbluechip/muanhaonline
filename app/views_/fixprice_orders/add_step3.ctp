		...
		<div class="left-content">
			...
			<!--ABOUT INTRO TEXT-->
					<div class="motabds">
						...
						<ul>
							<li>Tên khách hàng: <strong><?php echo $fixprice_order['FixpriceCustomer']['name'] ?></strong></li>
							<li>Email: <strong><?php echo $fixprice_order['FixpriceCustomer']['email'] ?></strong></li>
							<li>Số điện thoại: <strong><?php echo $fixprice_order['FixpriceCustomer']['phone'] ?></strong></li>
							<li>Địa chỉ: <strong><?php echo $fixprice_order['FixpriceCustomer']['address'] ?></strong></li>
			...
			<!--MISSION AND VISION-->
					...
					<?php if($product["Product"]["longitude"].$product["Product"]["latitude"] != '') { ?>
						<div class="vitri" style="height:auto;clear:both">
								...
								<div id="google-map">
									<div id="map_canvas" style="width:571px; height:300px; border:solid 2px #cccccc;"></div>
								</div>
						</div>
					<?php } ?>
					<?php if(!$return_customer) { ?>
						<div class="backstep fillupform" style="width: 100%; clear: both">
						   <a href="<?php echo $this->Html->url(array('controller'=>'fixprice_orders', 'action'=>'add_step2')) ?>"><< Quay lại bước 2 (Mô tả lại BĐS)</a>
						</div>
					<?php } ?>
			...
			<?php if($fixprice_order["FixpriceOrder"]["status"] == 1) { ?>
			<h2 class="fix_heading" style="">Chọn hình thức thanh toán</h2>
				<div class="thongtincanho">
					...
					(Visa, MasterCard, ATM, ...)
				     ...
				     <?php if($fixprice_order['FixpriceType']['id'] == 1) { ?>
					...
					<p>Nhắn <strong>MNO 000<?php echo $fixprice_order['FixpriceOrder']['id'] ?></strong> gửi về <strong>8683</strong></p>										
				    </div>
				     <?php } ?>
				    <?php if(!$return_customer) { ?>
					<div class="pay_later fillupform ">
					    <?php echo $this->Form->create('Product', array('url' => array('controller' => 'fixprice_orders', 'action' => 'add_step3')));?>
						    <input type="submit" class="btsubmit" name="save_later" value="Lưu yêu cầu" />
					    <?php echo $this->Form->end();?>
					    <br />
					    (Lưu thông tin yêu cầu và thanh toán sau)
					</div>
				    <?php } ?>
				</div>
                        <?php } ?>

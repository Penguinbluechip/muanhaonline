...
class ProductsController extends AppController {
	...
	function index()
	{
		...
		//not show product for fixprice
		$conditions["Product.is_fixprice"] = "0";
		...
		foreach($products as $key => $p)
		{
			...
			$products[$key]["Product"]["sname"] = $this->Product->getProductTitle($p);
		...
		$this->set('page_title', "Bất động sản - MuaNhaOnline.vn");
	...
	function details($id = null)
	{
		...
		foreach($relatedProducts as $key => $p)
		{
		    ...
		    $relatedProducts[$key]["Product"]["name"] = $this->Product->getProductTitle($p);
		...
		$product["Product"]["titlex"] = $product["Product"]["name"];
		//$product["Product"]["sname"] = $this->Product->getProductTitle($product);
		//$product["Product"]["name"] = $this->Product->getProductTitle($product);
		
		//var_dump($product["Product"]["meta_keywords"]);
		
		//meta
		if($product["Product"]["meta_keywords"])
		{
			$this->set('meta_keywords', $product["Product"]["meta_keywords"]);
		}
		//if($product["Product"]["meta_description"])
		//{
			$this->set('meta_description', substr(trim(preg_replace('/\s+/', ' ', strip_tags($product["Product"]["description"]))),0,250));
		//}
		...
		$this->set('pus', $pus);
		$this->set('page_title', $product["Product"]["titlex"]);
	}
	...
	function admin_publish($id = null) {
		...
		$product = $this->Product->read(null, $id);
		    ...
		    $product["Product"]["published_date"] = $today;
	...
	function manager_renew($id = null) {
		...
		if($product["Product"]["published"] == 2 || $product["Product"]["published"] == 3 || $product["Product"]["published"] == -1)
		    ...
		    $product["Product"]["published_date"] = $today;
		    $product["Product"]["renew_count"] = ++$product["Product"]["renew_count"];
		    $product["Product"]["expire_date"] = date('Y-m-d H:i:s', strtotime($today)+(14*24*3600));

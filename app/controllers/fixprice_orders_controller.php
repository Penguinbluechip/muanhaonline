...
class FixpriceOrdersController extends AppController {
	...
	function add_step1() {
		...
		if (!empty($this->data)) {	
				...
				$saved = $this->FixpriceOrder->save($this->data);
				if ($saved) {
			...
			//STEP 2 SAVE BDS
			if ($saved) {				
				$fixprice_order = $this->FixpriceOrder->read(null, $this->FixpriceOrder->id);
				
				//set for fixprice product
				$this->data["Product"]["is_fixprice"] = 1;
				...
				if(!isset($this->params['form']['save_later'])) {				
					...
					if ($this->Product->save($this->data)) {
						...
						if($this->FixpriceOrder->save($fixprice_order['FixpriceOrder']))
						{
							...
							{
								$this->Session->setFlash(__('Hồ sơ đăng ký hoàn tất!<br />Quý khách tiến hàng thanh toán phí dịch vụ bên dưới sau khi xem lại đầy đủ thông tin của mình', true));
							}
	...
	function admin_setpaidnl($order_id = null) {
		...
		if(true)
		{
			...
			$fixprice_order = $this->FixpriceOrder->read(null, $order_id);
						...
						//echo $fixprice_order['FixpriceCustomer']['name'];
						$this->set('fixprice_order', $fixprice_order);
						$link = FULL_BASE_URL.Router::url(array('controller'=>'fixprice_orders', 'action'=>'add_step3', $fixprice_order["FixpriceOrder"]["id"], $fixprice_order["User"]["email"], 'admin'=>false), false);
						//preg_match('/{title}(.*?){\/title}/', $mail_content['Content']['content'], $title);
						//$this->set('mail_content', $mail_content);
	...
	function order_result($order_id = '', $customer_email = '') {
		...
		else
		{
			//check customer with email
			$fixprice_order = $this->FixpriceOrder->read(null, $order_id);
			if($fixprice_order['User']['email'] != $customer_email)
			{
				$this->Session->setFlash(__('Truy cập không đúng', true));
				//$this->redirect(array('action' => 'add_step1'));
			}
		...
		if($state != "VALID" && $state != "FINISHED_RATED" && $state != "PUBLISHED")
		{
	...
	function admin_index() {
		...
		$this->paginate = array('conditions'=>$conditions, 'order'=>'FixpriceOrder.create_date DESC');
		...
		foreach($fixpriceOrders as $key => $item)
		{
			...
			$fixpriceOrders[$key]['state'] = $this->FixpriceOrder->getState($item['FixpriceOrder']['id']);
		}
	...
	function manager_index()
	{
		...
		foreach($fixpriceOrders as $key => $item)
		{
			...
			//Product title
			$fixpriceOrders[$key]['Product']['name'] = $this->FixpriceOrder->Product->getProductTitle($item['Product']['id'], true);
		}
	...
	function user_index()
	{
		...
		{
			$a_order_ids = $this->FixpriceOrder->getByUser(array('VALID', 'PUBLISHED'), $user['User']['id']);
		}
		...
		{
			$a_order_ids = $this->FixpriceOrder->getByUser(array('PAID', 'REGISTERED', ' PRIVATE_REGISTER', 'ASSIGNED', 'EXPERT_CONFIRMED', 'EXPERT_REJECTED', 'EXPERT_PENDING', 'INVALID'), $user['User']['id']);
		}
		$conditions["FixpriceOrder.id"] = $a_order_ids;
	...
	function expert_index() {
		...
		//$this->FixpriceOrder->updateStatusFailed();
		$this->FixpriceOrder->updateFixpriceOrdersStatus();
		...
		else if($status == 'EXPERT_CONFIRMED_LEAD')
		{
			$a_order_ids = $this->FixpriceOrder->getByGroupLeader($user['User']['id'], array('EXPERT_CONFIRMED'));
		}
		else if($status == 'EXPERT_REJECTED_LEAD')
		{
			$a_order_ids = $this->FixpriceOrder->getByGroupLeader($user['User']['id'], array('EXPERT_REJECTED'));
		}
		else if($status == 'EXPERT_CONFIRMED' || $status == 'EXPERT_REJECTED' || $status == 'EXPERT_PENDING' || $status == 'INVALID' || $status == 'INSPECTOR_CONFIRMED')
		{
			$a_order_ids = $this->FixpriceOrder->getByExpert($user['User']['id'], $status);
		}
		else if($status == 'VALID')
		{
			$a_order_ids = $this->FixpriceOrder->getByExpert($user['User']['id'], array('VALID', 'INSPECTOR_CONFIRMED_PENDING_ERROR'));
		}
		else if($status == 'ASSIGNED')
		{
			$a_order_ids = $this->FixpriceOrder->getByAssigner($user['User']['id'], 'EXPERT_CONFIRMED');
		}
		else if($status == 'WORKING')
		{
			$a_order_ids = $this->FixpriceOrder->getByExpert($user['User']['id'], array('EXPERT_CONFIRMED','EXPERT_PENDING','INVALID'));
		}
		else if($status == 'FAILED')
		{
			$a_order_ids = $this->FixpriceOrder->getByExpert($user['User']['id'], array('INSPECTOR_CONFIRMED', 'PUBNISHED', 'FAILED'));
		}
		else if($status == 'FINISHED_ALL')
		{
			$a_order_ids = $this->FixpriceOrder->getByExpert($user['User']['id'], array('VALID','FINISHED_RATED'));
		}
		else if($status == 'WORKING_LEAD')
		{
			$a_order_ids = $this->FixpriceOrder->getByGroupLeader($user['User']['id'], array('EXPERT_CONFIRMED','EXPERT_PENDING','INVALID'));
		}
		else if($status == 'FAILED_LEAD')
		{
			$a_order_ids = $this->FixpriceOrder->getByGroupLeader($user['User']['id'], array('INSPECTOR_CONFIRMED', 'PUBNISHED', 'FAILED'));
		}
		else if($status == 'FINISHED_ALL_LEAD')
		{
			$a_order_ids = $this->FixpriceOrder->getByGroupLeader($user['User']['id'], array('VALID','FINISHED_RATED'));
		}
		else
		{
			$a_order_ids = $this->FixpriceOrder->getByExpert($user['User']['id'], $status);
		...
		$private_expert_confirm_time = $this->Setting->get('private_expert_confirm_time');
		
		$private_expert_rejected_time = $this->Setting->get('private_expert_rejected_time');
		...
		foreach($fixpriceOrders as $key => $item)
		{
			...
			else if($state['FixpriceOrderState']['alias'] == 'INSPECTOR_CONFIRMED_PENDING_ERROR')
			{
				$remain_hours = 0;									
			}
			...
			//////////////////////
			if($remain_hours != 0 && strtotime($state['FixpriceOrdersState']['created_date'])+$remain_hours > strtotime(date('Y-m-d H:i:s')))
			{
				//if(strtotime($state['FixpriceOrdersState']['created_date'])+$remain_hours > strtotime(date('Y-m-d H:i:s')))
				//{
					$remain_stat = round((((strtotime(date('Y-m-d H:i:s')) - strtotime($state['FixpriceOrdersState']['created_date'])))/$remain_hours), 2);				
					$remain_hours = $this->dateDiff(date('Y-m-d H:i:s', strtotime($state['FixpriceOrdersState']['created_date'])+$remain_hours), date('Y-m-d H:i:s'));
					if($remain_stat > 0.66) $remain_stat=3; elseif($remain_stat > 0.33) $remain_stat=2; else $remain_stat=1;
				//}
			}
			else
			{
			...
			$fixpriceOrders[$key]['remain_stat'] = $remain_stat;
			
			$remain_hours = str_replace('hours', 'giờ', $remain_hours);
			$remain_hours = str_replace('minutes', 'phút', $remain_hours);
			$remain_hours = str_replace('seconds', 'giây', $remain_hours);
			$fixpriceOrders[$key]['remain_hours'] = $remain_hours;
			
			
			
			//Product title
			$fixpriceOrders[$key]['Product']['name'] = $this->FixpriceOrder->Product->getProductTitle($item['Product']['id'], true);
			$fixpriceOrders[$key]['FixpriceOrdersState'] = $state['FixpriceOrdersState'];
			//$fixpriceOrders[$key]['Product']['name'] = $this->FixpriceOrder->Product->getProductTitle($item['Product']['id'], true);
	...
	function expert_postanswer($fixprice_order_id) {
		...
		if($order_state == 'EXPERT_CONFIRMED' || $order_state == 'INVALID')
		{
			...
			if ($this->FixpriceOrder->save($fixpriceOrder)) {
				...
				$this->FixpriceOrder->setState($fixpriceOrder['FixpriceOrder']['id'], 'EXPERT_PENDING',"post answer", $fixpriceOrder["FixpriceOrder"]["expert_id"]);
	...
	function expert_resorder($id = null)
	{
		...
		if (!empty($this->data)) {
			...
			$this->data["FixpriceOrder"]["expert_id"] = $user['User']['id'];
			
			if($state == 'PAID') {
				if ($this->FixpriceOrder->save($this->data)) {
					$this->FixpriceOrder->setState($id, 'ASSIGNED', 'auto', $user["User"]["id"]);
					$this->FixpriceOrder->setState($id, 'EXPERT_CONFIRMED', 'auto', $user["User"]["id"]);
	...
	function expert_assign($id = null) {
		...
		if (!empty($this->data)) {
			...
			if(!isset($this->data['FixpriceOrder']['assigner_id']))
			{
			...
			if ($this->FixpriceOrder->save($this->data)) {
				...
				//Set Product State
				if($this->FixpriceOrder->getState($this->FixpriceOrder->id) == "EXPERT_CONFIRMED")
				{
					$this->FixpriceOrder->setState($this->FixpriceOrder->id, 'EXPERT_CONFIRMED', $this->data['FixpriceOrdersState']['note'], $this->data['FixpriceOrder']['assigner_id'], true);
				}
				
				//Send thanks email to expert
				//Sending email
				$expert = $this->FixpriceOrder->Expert->read(null, $this->data['FixpriceOrder']['assigner_id']);
		...
		if (empty($this->data)) {
			...
			$this->data['Product']['name'] = $this->data['FixpriceService']['name']." - ".$product['Product']['facade']." - ".$product['Product']['home_number']." ".$product['Street']['name'].", ".$product['Ward']['name'].", ".$product['District']['name'].", ".$product['City']['name'];
		}
		...
		$this->set("assigners", $experts);
	}
	
	function expert_assign_confirm($id = null) {
		Controller::loadModel('DistrictsExpert');
		Controller::loadModel('Content');
		
		$user = $this->Auth->user();
		$user = $this->FixpriceOrder->User->read(null, $user["User"]["id"]);
		//var_dump($user["ExpertGroup"][0]["ExpertGroupsUser"]["expert_group_id"]);
		...
		if (!empty($this->data)) {
			...
			//Assign user id for fixprice order
			$this->data['FixpriceOrder']['expert_id'] = $user["User"]["id"];
			$this->data['FixpriceOrder']['expert_group_id'] = $user["ExpertGroup"][0]["ExpertGroupsUser"]["expert_group_id"];
			$this->data['FixpriceOrder']['status'] = 3;
			$this->data['FixpriceOrder']['assign_date'] = date('Y-m-d H:i:s');
			
			if ($this->FixpriceOrder->save($this->data)) {
				$this->Session->setFlash(__('Đăng ký thẩm định thành công', true));
				
				//Set Product State
				$this->FixpriceOrder->setState($this->FixpriceOrder->id, 'ASSIGNED', $this->data['FixpriceOrdersState']['note'], $this->data['FixpriceOrder']['expert_id']);
				$this->FixpriceOrder->setState($this->FixpriceOrder->id, 'EXPERT_CONFIRMED', $this->data['FixpriceOrdersState']['note'], $this->data['FixpriceOrder']['expert_id']);
	...
	function expert_confirm()
	{		
		if (!empty($this->data)) {
			
			$fixprice_order_id = $this->data['FixpriceOrder']['id'];
			
			$user = $this->Auth->user();		
			$fixpriceOrder = $this->FixpriceOrder->find('first', array('conditions'=>array(
													'FixpriceOrder.id'=>$fixprice_order_id,
													'FixpriceOrder.assigner_id'=>$user['User']['id']
												)));
			...
			$order_state = $this->FixpriceOrder->getState($fixpriceOrder['FixpriceOrder']['id']);
			if ($order_state != 'EXPERT_CONFIRMED' && $user["User"]["id"] == $fixpriceOrder['FixpriceOrder']['assigner_id']) {
				$this->Session->setFlash(__('Invalid fixprice order', true));
				$this->redirect(array('controller'=>'fixprice_orders','action' => 'index'));
			}
			
			
			//save fixprice order with new expert
			$fixpriceOrder["FixpriceOrder"]["expert_id"] = $user["User"]["id"];
			$fixpriceOrder["FixpriceOrder"]["assigner_id"] = null;
			$this->FixpriceOrder->save($fixpriceOrder);
			
			if($this->FixpriceOrder->setState($fixpriceOrder['FixpriceOrder']['id'], 'EXPERT_CONFIRMED', $this->data['FixpriceOrdersState']['note'], $user["User"]["id"], true))
			{
				$this->Session->setFlash(__('Xác nhận thẩm định thành công.', true));
			}
			else
			{
				$this->Session->setFlash(__('Đã có lỗi trong quá trình xác nhận.', true));
			}
		}
		$this->redirect(array('action' => 'index'));
	}
	
	function expert_reject()
	{
		if (!empty($this->data)) {
			...
			$order_state = $this->FixpriceOrder->getState($fixpriceOrder['FixpriceOrder']['id']);
			if ($order_state != 'EXPERT_CONFIRMED') {
				$this->Session->setFlash(__('Invalid fixprice order', true));
				$this->redirect(array('controller'=>'fixprice_orders','action' => 'index'));
			}
			...
			if($this->FixpriceOrder->setState($fixpriceOrder['FixpriceOrder']['id'], 'EXPERT_CONFIRMED', $this->data['FixpriceOrdersState']['note'], $user["User"]["id"], true))
			{
				...
				$this->Session->setFlash(__('Từ chối thẩm định thành công.', true));
			}
			else
			{
				$this->Session->setFlash(__('Đã có lỗi trong quá trình xác nhận.', true));
	...
	function expert_assignleader()
	{
		if (!empty($this->data)) {
			...
			if(!$this->FixpriceOrder->isLeaderGroup($user['User']['id'], $fixpriceOrder['ExpertGroup']['id']))
			{
				$this->Session->setFlash(__('Người dùng không phù hợp'.$user['User']['id'].$fixpriceOrder['ExpertGroup']['id'], true));
				$this->redirect(array('controller'=>'fixprice_orders','action' => 'index'));
			}
			
			
			$order_state = $this->FixpriceOrder->getState($fixpriceOrder['FixpriceOrder']['id']);
			if ($order_state != 'EXPERT_REJECTED') {
				$this->Session->setFlash(__('Invalid fixprice order', true));
				$this->redirect(array('controller'=>'fixprice_orders','action' => 'index'));
			}
			...
			$fixpriceOrder['FixpriceOrder']['expert_id'] = $user['User']['id'];
			if ($this->FixpriceOrder->save($fixpriceOrder)) {				
				if($this->FixpriceOrder->setState($fixpriceOrder['FixpriceOrder']['id'], 'EXPERT_CONFIRMED', $this->data['FixpriceOrdersState']['note']))
				{				
					$this->Session->setFlash(__('Xác nhận thẩm định thành công.', true));
				}
				else
	...
	{
		Controller::loadModel('Setting');
		Controller::loadModel('FixpriceOrdersState');
		$user = $this->Auth->user();
		...
		if($status == 'INSPECTOR_CONFIRMED')
		{
			$orders_array = array('INSPECTOR_CONFIRMED','INSPECTOR_CONFIRMED_PENDING_ERROR');
		}
		else
		{
			$orders_array = $status;
		...
		if($status != '' && $status != 'all')
		{
			//$conditions["FixpriceOrder.status"] = $status;
			//echo $status;
			$a_order_ids = $this->FixpriceOrder->getByState($orders_array);
			$conditions["FixpriceOrder.id"] = $a_order_ids;
		}
		...
		$inspector_expert_pending = $this->Setting->get('inspector_expert_pending');
		$inspector_confirmed_time = $this->Setting->get('inspector_confirmed_time');
		
		$inspector_confirm_wait = $this->Setting->get('inspector_confirm_wait');
		
		$full_fixprice_time = $this->Setting->get('full_fixprice_time');
		...
		foreach($fixpriceOrders as $key => $item)
		{
			...
			{
				$first_state = $this->FixpriceOrder->getStateByOrder($item['FixpriceOrder']['id'], "PAID");
				$remain_hours = round((strtotime($first_state["FixpriceOrdersState"]["created_date"]) + $full_fixprice_time - (strtotime(date('Y-m-d H:i:s'))))/3600, 2);
				$fixpriceOrders[$key]['remain_hours'] = $remain_hours." tiếng";			
			}
			else if($state['FixpriceOrderState']['alias'] == 'INSPECTOR_CONFIRM_WAIT')
			{
				$remain_hours = round(($inspector_confirm_wait - (strtotime(date('Y-m-d H:i:s')) - strtotime($state['FixpriceOrdersState']['created_date'])))/3600, 2);
				$fixpriceOrders[$key]['remain_hours'] = $remain_hours." tiếng";			
			}
			else
			{
			...
			//Product title
			$fixpriceOrders[$key]['Product']['name'] = $this->FixpriceOrder->Product->getProductTitle($item['Product']['id'], true);
			$fixpriceOrders[$key]['FixpriceOrder']['countinvalid'] = $this->FixpriceOrder->countInvalid($item['FixpriceOrder']['id']);
		}
			...
			if($state == 'EXPERT_PENDING')
			{
				...
				if ($this->FixpriceOrder->save($fixprice_order)) {
					$this->Session->setFlash(__('Thẩm định đã được đăng.', true));
							...
							$order_link = FULL_BASE_URL.Router::url(array('controller'=>'fixprice_orders', 'action'=>'order_result', $fixprice_order["FixpriceOrder"]["id"], $fixprice_order["User"]["email"], 'admin'=>false), false);
					...
					//$this->FixpriceOrder->User->addPoint($fixprice_order['FixpriceOrder']['expert_id'], $this->Setting->get('valid_order_point'));
					//echo $user."/".$this->Setting->get('valid_order_point');
	...
	function inspector_confirm($id = null)
	{
		if (!$id) {
			$this->Session->setFlash(__('Invalid id for fixprice order', true));
			$this->redirect(array('action'=>'index'));
		...
		if($state == 'INSPECTOR_CONFIRM_WAIT')
		{
			$this->FixpriceOrder->setState($fixprice_order['FixpriceOrder']['id'], 'INSPECTOR_CONFIRMED', 'auto');
		}
		$this->Session->setFlash(__('Đã xác nhận thành công.', true));
		$this->redirect(array('action' => 'index'));
	}
		...
		if (!empty($this->data)) {
			...
			if($state == 'EXPERT_PENDING')
			{
				...
				if ($this->FixpriceOrder->save($fixprice_order)) {
					...
					$this->FixpriceOrder->setState($fixprice_order['FixpriceOrder']['id'], 'INVALID', "auto", $fixprice_order['FixpriceOrder']['expert_id']);
	...
	function inspector_register() {
		if (!empty($this->data)) {
			...
			if($this->FixpriceOrder->setState($fixpriceOrder['FixpriceOrder']['id'], 'INSPECTOR_CONFIRMED', $this->data['FixpriceOrdersState']['note'], $user['User']['id']))
			{
				$this->Session->setFlash(__('Xác nhận thẩm định thành công.', true));
			}
			else
	...
	function inspector_publish($fixprice_order_id) {
		if (!$fixprice_order_id && empty($this->data)) {
			$this->Session->setFlash(__('Invalid fixprice order', true));
			$this->redirect(array('controller'=>'fixprice_orders','action' => 'index'));
		}
		...
		if($order_state == 'INSPECTOR_CONFIRMED')
		{
			...
			if ($this->FixpriceOrder->save($fixpriceOrder)) {
				...
				$this->FixpriceOrder->setState($fixpriceOrder['FixpriceOrder']['id'], 'PUBLISHED');

	...
	function ajaxStreetOption($district_id)
	{
		...
		else
		{
			...
			if($street)
			{
				$joins = array(
					array(
					  'table' => 'hk_districts_streets',
					  'alias' => 'DT',
					  'type' => 'LEFT',
					  'conditions' => array('Street.id = DT.street_id')
					)
				);
				$streets = $this->Street->find("all", array('joins' => $joins, 'conditions'=>array('DT.district_id'=>$district_id)));
				//var_dump($streets[0]);

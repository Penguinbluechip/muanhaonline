...
class FixpriceOrdersController extends AppController {
	...
	function add_step3($order_id = '', $customer_email = '') {
		$this->layout = 'home';
		...
		if(isset($this->params['form']['save_later']))
		{
						...
						$this->SwiftMailer->to = $fixprice_order['FixpriceCustomer']['email'];
	...
	function order_result($order_id = '', $customer_email = '') {
		...
		if($fixprice_order["FixpriceOrder"]["status"] != 5)
		{
	...
	function manager_index()
	{
		...
		foreach($fixpriceOrders as $key => $item)
		{
			if($item['FixpriceOrder']['status'] == 1)
			{
	...
	function expert_index() {
		...
		//Get all filters
		$conditions = array('FixpriceOrder.expert_id'=>$user["User"]["id"]);
		...
		foreach($fixpriceOrders as $key => $item)
		{
			if($item['FixpriceOrder']['assign_date'])
			{
				if($item['FixpriceOrder']['status'] == 3)
					$fixpriceOrders[$key]['remain_hours'] = 12 - round((strtotime(date('Y-m-d H:i:s')) - strtotime($item['FixpriceOrder']['assign_date']))/3600, 0);
				else if($item['FixpriceOrder']['status'] == -1)
					$fixpriceOrders[$key]['remain_hours'] = 18 - round((strtotime(date('Y-m-d H:i:s')) - strtotime($item['FixpriceOrder']['assign_date']))/3600, 0);
			}
		}

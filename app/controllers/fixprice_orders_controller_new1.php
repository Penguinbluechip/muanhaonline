...
class FixpriceOrdersController extends AppController {
	...
	function expert_index() {
		...
		else if($status == 'ASSIGNED' || $status == 'EXPERT_CONFIRMED' || $status == 'EXPERT_REJECTED' || $status == 'EXPERT_PENDING' || $status == 'VALID' || $status == 'INVALID' || $status == 'INSPECTOR_CONFIRMED')
		{
		...
		{
			$a_order_ids = $this->FixpriceOrder->getByGroupLeader($user['User']['id'], array('INVALID','FINISHED_RATED'));
		}
		else
		{
			$a_order_ids = $this->FixpriceOrder->getByExpert($user['User']['id'], $status);
		...
		foreach($fixpriceOrders as $key => $item)
		{
			...
			else if($state['FixpriceOrderState']['alias'] == 'EXPERT_CONFIRMED' || $state['FixpriceOrderState']['alias'] == 'EXPERT_REJECTED')
			{
				$remain_hours = $private_expert_confirm_time;						
			}
			else if($state['FixpriceOrderState']['alias'] == 'EXPERT_REJECTED' || $state['FixpriceOrderState']['alias'] == 'EXPERT_REJECTED')
			{
	...
	function expert_reject()
	{
		if (!empty($this->data)) {
			...
			if($this->FixpriceOrder->setState($fixpriceOrder['FixpriceOrder']['id'], 'EXPERT_REJECTED', $this->data['FixpriceOrdersState']['note']))
			{				
				$this->Session->setFlash(__('Xác nhận thẩm định thành công.', true));
			}
			else
	...
	function inspector_index()
	{
		...
		foreach($fixpriceOrders as $key => $item)
		{
			...
			else if($state['FixpriceOrderState']['alias'] == 'INSPECTOR_CONFIRMED')
			{
				$remain_hours = round(($inspector_confirmed_time - (strtotime(date('Y-m-d H:i:s')) - strtotime($state['FixpriceOrdersState']['created_date'])))/3600, 2);
				$fixpriceOrders[$key]['remain_hours'] = $remain_hours." tiếng";			
			}
			else
			{
